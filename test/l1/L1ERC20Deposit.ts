...
describe('L1ERC20Deposit', () => {
  ...
  before(async () => {
    ...
    Mock__OVM_L2DepositedERC20 = await smockit(await ethers.getContractFactory('L2ERC20Minter'))

    // deploy an ERC20 contract on L1
    Factory__L1ERC20 = (await smoddit('Dai')) as any
  ...
  describe('finalizeWithdrawal', () => {
    ...
    it('onlyFromCrossDomainAccount: should revert on calls from the right crossDomainMessenger, but wrong xDomainMessageSender (ie. not the L2ERC20Gateway)', async () => {
      Mock__OVM_L1CrossDomainMessenger.smocked.xDomainMessageSender.will.return.with(() => NON_ZERO_ADDRESS)
    ...
    it('should credit funds to the withdrawer and not use too much gas', async () => {
      ...
      // seed some funds
      await L1ERC20.transfer(await escrow.getAddress(), withdrawalAmount)

      const res = await OVM_L1ERC20Gateway.finalizeWithdrawal(NON_ZERO_ADDRESS, withdrawalAmount, {
        from: Mock__OVM_L1CrossDomainMessenger.address,
      })
      ...
      const OVM_L2DepositedERC20 = await (await ethers.getContractFactory('L2ERC20Minter')).deploy(
        ZERO_ADDRESS,
        ZERO_ADDRESS,
      )
  ...
  describe('deposits', () => {
    ...
    it('deposit() escrows the deposit amount and sends the correct deposit message', async () => {
      ...
      // escrow's balance is increased
      const escrowBalance = await L1ERC20.balanceOf(await escrow.getAddress())
      expect(escrowBalance).to.equal(depositAmount)

      // Check the correct cross-chain call was sent:
      // Message should be sent to the L2ERC20Gateway on L2
      expect(depositCallToMessenger._target).to.equal(Mock__OVM_L2DepositedERC20.address)
      ...
      expect(depositCallToMessenger._message).to.equal(
        await Mock__OVM_L2DepositedERC20.interface.encodeFunctionData('finalizeDeposit', [depositer, depositAmount]),
      )
    ...
    it('depositTo() escrows the deposit amount and sends the correct deposit message', async () => {
      ...
      // the L1 gateway sends the correct message to the L1 messenger
      expect(depositCallToMessenger._message).to.equal(
        await Mock__OVM_L2DepositedERC20.interface.encodeFunctionData('finalizeDeposit', [bobsAddress, depositAmount]),
      )
      expect(depositCallToMessenger._gasLimit).to.equal(finalizeDepositGasLimit)
    })
  })
