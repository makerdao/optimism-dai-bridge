...
describe('L1Gateway', () => {
  describe('depositERC20()', () => {
    ...
    it('reverts when approval is too low', async () => {
      const [l1MessengerImpersonator, user1] = await ethers.getSigners()
      const { l1Dai, l1Gateway, l2Dai } = await setupTest({
        l1MessengerImpersonator,
        user1,
      })
      ...
      await expect(
        l1Gateway.connect(user1).depositERC20(l1Dai.address, l2Dai.address, depositAmount, defaultGas, defaultData),
    ...
    it('reverts when funds too low', async () => {
      ...
      await expect(
        l1Gateway.connect(user2).depositERC20(l1Dai.address, l2Dai.address, depositAmount, defaultGas, defaultData),
  ...
  describe('finalizeERC20Withdrawal', () => {
    ...
    it('sends funds from the escrow', async () => {
      const [l1MessengerImpersonator, user1, user2] = await ethers.getSigners()
      const { l1Dai, l1Gateway, l2Dai, l1CrossDomainMessengerMock, l2GatewayMock, l1Escrow } = await setupWithdrawTest({
        l1MessengerImpersonator,
        user1,
      })
    ...
    it('completes withdrawals even when closed', async () => {
      const [l1MessengerImpersonator, user1, user2] = await ethers.getSigners()
      const { l1Dai, l2Dai, l1Gateway, l1CrossDomainMessengerMock, l2GatewayMock, l1Escrow } = await setupWithdrawTest({
        l1MessengerImpersonator,
        user1,
      })
      l1CrossDomainMessengerMock.smocked.xDomainMessageSender.will.return.with(() => l2GatewayMock.address)
    ...
    it('reverts when called not by XDomainMessenger', async () => {
      const [l1MessengerImpersonator, user1, user2] = await ethers.getSigners()
      const { l1Gateway, l1CrossDomainMessengerMock, l2GatewayMock, l1Dai, l2Dai } = await setupWithdrawTest({
        l1MessengerImpersonator,
        user1,
      })
    ...
    it('reverts when called by XDomainMessenger but not relying message from l2Gateway', async () => {
      ...
      const { l1Gateway, l1CrossDomainMessengerMock, l1Dai, l2Dai } = await setupWithdrawTest({
        l1MessengerImpersonator,
        user1,
      })
